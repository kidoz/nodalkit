...
std::string_view Label::text() const { return impl_->text; }
...
HAlign Label::h_align() const { return impl_->h_align; }
void Label::set_h_align(HAlign align) { impl_->h_align = align; }

SizeRequest Label::measure(Constraints const& /*constraints*/) const {
    FontDescriptor font;
    font.size = has_style_class("heading") ? 18.0F : 13.5F;
    font.weight = has_style_class("heading") ? FontWeight::Medium
                                             : FontWeight::Regular;
    auto const measured = measure_text(impl_->text, font);
    float const w = measured.width;
    float const h = std::max(
